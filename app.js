...
server.get("/api/films", [
  ...
  try {
    ...
    if(tri == "titre" || tri == "annee" || tri == "realisation"){
      const donneesRef = await db.collection("films").orderBy(tri, direction).limit(limit).get();
...
//      req deja netoyee
  ...
  // On enregistre (objet)
  const nouvelUtilisateur = { // quand key et value dans objet sont pareils (et value est var), on peut juste ecrire le  value
    "courriel":courriel, 
    "mdp": mdp
  };
